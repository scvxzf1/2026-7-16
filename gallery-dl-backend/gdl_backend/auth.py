...
class AuthManager:
    ...
    def _profile_path(self, site: str) -> Path:
        if site not in MANAGED_BROWSER_SITES:
            raise AuthError("managed_browser_unsupported", f"{site} 不使用项目专属浏览器授权")
        return (self.browser_profiles_dir / site).resolve()
    ...
    def status(self, site: str) -> dict[str, Any]:
        ...
        if site == "pixiv":
            ...
            session_state = session.state if session and session.process.returncode is None else ""
            if token:
                state = "authorized"
                summary = "Pixiv 登录授权有效；gallery-dl 会自动使用托管 Token。"
            elif session_state in {"starting", "awaiting_code", "exchanging"}:
                state = "authorizing"
                summary = "Pixiv 授权流程正在进行。"
            else:
                state = "required"
            ...
            return {
                ...
                "oauth": session.public_dict() if session_state else None,
                "actions": ["oauth", "clear"],
            }
        ...
        if session_active:
            state = "authorizing"
            summary = "项目专属浏览器已打开；请在窗口内完成登录。"
        elif authorized:
            state = "authorized"
            summary = f"{COOKIE_LABELS[site]}已持久化托管，后续自动复用。"
        elif invalidated:
            state = "required"
            summary = "登录凭证在实际访问中失效，请重新授权。"
        else:
            state = "required"
            summary = "完成一次项目专属浏览器登录后，凭证会自动持久化。"
        return {
            "site": site,
    ...
    async def _shutdown_browser_process(self, session: ManagedBrowserLoginSession) -> None:
        if session.websocket_url:
            await asyncio.to_thread(close_browser, session.websocket_url)
        if session.process.returncode is None:
            try:
    ...
    async def _dispose_browser_session(
        ...
        *,
        ...
        await self._shutdown_browser_process(session)
        ...
        try:
            while time.time() < session.expires_at:
                if session.process.returncode is not None:
                    raise RuntimeError("项目专属浏览器窗口已提前关闭")
                endpoint = await asyncio.to_thread(read_browser_websocket, session.debug_port)
                if endpoint:
                    _port, session.websocket_url = endpoint
                    break
                await asyncio.sleep(min(poll, 0.25))
            if not session.websocket_url:
                raise TimeoutError("项目专属浏览器启动超时")

            await asyncio.to_thread(clear_site_cookies, session.websocket_url, spec)
            await asyncio.to_thread(open_login_target, session.websocket_url, spec.login_url)
            session.state = "awaiting_login"
            session.message = f"请在项目专属浏览器中完成 {spec.label} 登录。"
            ...
            while time.time() < session.expires_at:
                ...
                if session.process.returncode is not None:
                    raise RuntimeError("项目专属浏览器窗口已提前关闭")
                cookies = await asyncio.to_thread(get_site_cookies, session.websocket_url, spec)
                ...
                if time.time() < session.expires_at and page_ready:
                    ...
                    session.message = "登录成功，凭证已持久化；以后会自动复用。"
                    return
                await asyncio.sleep(poll)

            session.state = "timed_out"
            session.error = "登录等待超时，请重新打开登录窗口。"
            ...
            if session.state != "cancelled":
                ...
                session.error = str(exc)[:500] or "项目专属浏览器登录失败"
                session.message = session.error
        finally:
            await self._shutdown_browser_process(session)
    ...
    async def start_browser_login(self, site: str) -> dict[str, Any]:
        ...
        try:
            ...
            raise AuthError("chrome_not_found", "没有找到 Google Chrome，请配置 auth.chrome_executable") from exc

        async with self._lock:
            old = self._latest_browser_session(site)
            if old and old.state in {"starting", "awaiting_login"}:
                await self._dispose_browser_session(old)
            self._browser_sessions = {
                session_id: session
                for session_id, session in self._browser_sessions.items()
                if session.site != site
            }
            profile_dir = self._profile_path(site)
            ...
            try:
                ...
                raise AuthError("browser_profile_busy", "项目专属浏览器配置正在使用中") from exc
            ...
            try:
                ...
                raise AuthError("browser_login_start_failed", "项目专属浏览器启动失败") from exc
    ...
    async def complete_pixiv_oauth(self, session_id: str, value: str) -> dict[str, Any]:
        ...
        async with self._lock:
            ...
            try:
                ...
                self._pixiv_session = None
                await self._dispose_pixiv_session(session, state="failed")
                raise AuthError("pixiv_oauth_process_ended", "Pixiv 授权输入通道已经关闭") from exc
    ...
    async def clear(self, site: str) -> dict[str, Any]:
        ...
        if site in MANAGED_BROWSER_SITES:
            profile_dir = self._profile_path(site)
            if profile_dir == (self.browser_profiles_dir / site).resolve():
